...
make_pheatmap_LR_data1 <- function(cci_conserved_results,
                                  seurat_object_sp1,
                                  seurat_object_sp2,
                                  species_name1 = "mm",
                                  species_name2 = "zf",
  ...
  write.table(merge_width_df, "lr_avg_df_two_species.txt")

  merge_width_df1 <- merge_width_df
  merge_width_df1$LR <- rep(merge_width_df$LR[seq(1,nrow(merge_width_df), 2)], each = 2)
  ...
  return(list(merge_width_df=merge_width_df_width, specie_conserved_CCC_sp1=specie_conserved_CCC_sp1, specie_conserved_CCC_sp2=specie_conserved_CCC_sp2))
}
...
#' @param assay_set Assay to get the expression, like "RNA", or "SCT"
#'
#' @return
#' @export
#'
...
#'
#' @param width_df width dataframe, result of function make_pheatmap_LR_data1
...
pheatmap_LR1 <- function(width_df,
  ...
  if (is.null(species_colors)) {
    species_colors <-c(rgb(0/255,114/255,189/255), rgb(248/255,115/255,106/255))
  }
  names(species_colors) <- c(species_name1, species_name2)
