...
In this part, CACIMAR first uses function Identify_SharedMarkers to calculate the power of each cluster pair to identify cross-species cell types. Power of each cluster pair is calculated as:

$$ (P_{s1} + P_{s2}) /(P_{1}+P_{2}) $$

where, $P_{s1}$ represents the sum of Power of genes shared by Cluster1 and Cluster2 in cluster1, $P_{s2}$ represents the sum of Power of genes shared by Cluster1 and Cluster2 in cluster2, $P_{1}$ represents the sum of Power of all genes in Cluster1, $P_{2}$ represents the sum of Power of all genes in Cluster2.

Then, CACIMAR integrates with R package pheatmap to visualize the result.
